...
int get_larger_bin_index(int size) {
  if (size < 1000) {
    return size / 100 + 1;
  }
  int index = 9 + size / 1000 + 1;
  if (index > 13) {
    index = 13;
  }
  return index;
...
void find_fit_metadata(int size, my_metadata_t **prev_p, my_metadata_t **metadata_p) {
  ...
  bool larger = true;
  int index = get_larger_bin_index(size); // 一回り大きいbinsのindexを返すまたは4000以上は14番目のbinsを探す
  if (bins[index].free_head->size == 0) {
    index = get_fit_bin_index(size);
    larger = false;
  }
