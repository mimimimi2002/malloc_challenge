...
void my_add_to_free_list(my_metadata_t *metadata) {
  ...
  if (metadata->size < SIZE_BOUNDARY) {
    ...
    metadata->next = b_bin.free_head;
    b_bin.free_head = metadata;
  }
}
...
void my_initialize() {
  a_bin.free_head = &a_bin.dummy;
  a_bin.dummy.size = 0;
  a_bin.dummy.next = NULL;
  b_bin.free_head = &b_bin.dummy;
  b_bin.dummy.size = 0;
  b_bin.dummy.next = NULL;
}
...
void find_fit_metadata(int size, my_metadata_t **prev_p, my_metadata_t **metadata_p) {
  ...
  // もしsizeが1000より小さかったら、a_binから探す
  if (size < SIZE_BOUNDARY) {
    prev = NULL;
...
void *my_malloc(size_t size) {
  ...
  my_metadata_t *metadata=a_bin.free_head;
  my_metadata_t *prev=a_bin.free_head;
