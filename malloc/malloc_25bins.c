...
//
my_heap_t bins[BIN_SIZE];

//
// Helper functions (feel free to add/remove/edit!)
//

int get_fit_bin_index(int size) {
  if (size < 128) {
    return size / 32;  // 0〜127 → 0〜3
  } else if (size < 256) {
    return 4 + (size - 128) / 128;
  } else if (size < 512) {
    return 8 + (size - 256) / 256;
  } else if (size < 1024) {
    return 12 + (size - 1024) / 1024;
  } else if (size < 2048) {
    return 16 + (size - 2048) / 2048;
  } else if (size < 4096) {
    return 20 + (size - 4096) / 4096;
  }
  return BIN_SIZE - 1;
}
...
void my_add_to_free_list(my_metadata_t *metadata) {
  ...
  // 25個のbinのうちどれに入れるかのindexをとってくる
  int index = get_fit_bin_index(metadata->size);
...
// sizeがfitする一番良いmetadataを取ってくる
void find_fit_metadata(int size, my_metadata_t **prev_p, my_metadata_t **metadata_p) {
  my_metadata_t *metadata=bins[0].free_head;
  my_metadata_t *prev=NULL;
  int index = get_fit_bin_index(size);

  while (index < BIN_SIZE - 2) {

    // metadataは最初のfree_headのさすものを持ってくる(ここでwhileを使って探さなくてもいい)
    prev = NULL;
  ...
  prev = NULL;
  metadata = bins[BIN_SIZE - 1].free_head;
